...
def get_page_post(page):
    cursor.execute(f"SELECT Number,Title,Writer_id,Datetime,View_Count,Recommend_Count FROM Post WHERE Is_Del = 0 ORDER BY Number DESC LIMIT {MAX_PAGE} OFFSET {page-1} * {MAX_PAGE};")
...
def add_post(title,writer_id,datetime,content):
    ...
    cursor.execute(f"INSERT INTO Post VALUES('{num+1}','{title}','{writer_id}','{datetime}',0,0,'{content}',0);")
    con.commit()
